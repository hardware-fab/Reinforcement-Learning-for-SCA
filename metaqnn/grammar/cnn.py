...
class Cnn(runtime.Parser):
    ...
    def input(self, _parent=None):
        _context = self.Context(_parent, self._scanner, 'input', [])
        INPUT = self._scan('INPUT', context=_context)
        result = ['input']
    ...
    def layers(self, _parent=None):
        ...
        _token = self._peek('CONV', 'NIN', 'GLOBALAVE', 'FLATTEN', 'BATCHNORM', 'POOL', 'SPLIT', 'FC', 'DROP',
                            'SOFTMAX', context=_context)
        if _token == 'CONV':
    ...
    def flat(self, _parent=None):
        _context = self.Context(_parent, self._scanner, 'flat', [])
        FLATTEN = self._scan('FLATTEN', context=_context)
        result = ['flat']
        numlist = self.numlist(_context)
        return result + numlist
